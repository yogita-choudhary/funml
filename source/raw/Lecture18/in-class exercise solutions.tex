...
A bottleneck limits capacity, so the AE must keep only information useful for reconstructing the input.
\[
...
More latent capacity typically makes reconstruction easier, so training reconstruction error tends to decrease (or not worsen).
\[
...
MSE penalizes per-pixel deviation; when uncertainty/multi-modality exists, it often yields averaged (blurry) outputs.
\[
\boxed{\textbf{Answer: (B)}}
\]
...
This is the key motivation for convolutional AEs on images.
\[
\boxed{\textbf{True}}
\]
...
The decoder is trained to reconstruct inputs from latent codes; it is not generally a strict inverse.
\[
\boxed{\textbf{Answer: (B)}}
\]
