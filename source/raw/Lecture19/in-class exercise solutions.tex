...
The $\ell_1$ penalty encourages sparsity in the latent activations, meaning only a few components of $\mathbf{z}$ are active.
\[
...
Higher $\lambda$ increases the sparsity pressure, often reducing active units but potentially harming reconstruction.
\[
\boxed{\textbf{Answer: (B)}}
\]
...
The encoder consumes the corrupted input $\tilde{\mathbf{X}}$, but the loss compares reconstruction to the clean data $\mathbf{X}$.
\[
\boxed{\textbf{Answer: (B)}}
\]
...
Training on corrupted inputs with a clean target encourages features that are stable under noise/corruption.
\[
\boxed{\textbf{Answer: (B)}}
\]
...
The reparameterization trick isolates randomness in $\boldsymbol{\epsilon}$, enabling gradient-based optimization of $\boldsymbol{\mu}$ and $\boldsymbol{\sigma}$.
\[
\boxed{\textbf{Answer: (B)}}
\]
...
% \vspace{10pt}
% \noindent
% \textbf{Final answers:} \fbox{(1) B \quad (2) B \quad (3) B \quad (4) B \quad (5) B}
